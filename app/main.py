...
# CSF 2.0 functions include GOVERN
CSF_FUNCTIONS = ["Govern", "Identify", "Protect", "Detect", "Respond", "Recover"]

NIST_KB = {
    "ransomware": ["Govern", "Identify", "Protect", "Detect", "Respond", "Recover"],
    "phishing":   ["Protect", "Detect", "Respond", "Recover", "Govern"],
    "unauthorized access": ["Detect", "Respond", "Recover", "Identify", "Govern"],
    "data breach": ["Govern", "Identify", "Protect", "Detect", "Respond", "Recover"],
    "surveillance": ["Govern", "Identify", "Protect", "Respond"],
    "ai-enabled": ["Govern", "Identify", "Detect", "Respond", "Recover"]
}
...
NIST_ACTIONS = {
    "Govern": [
        "Set risk appetite; define decision rights & escalation (counsel, LE, privacy, equity)",
        "Adopt/align policy (surveillance, AI use, data retention) and oversight processes",
        "Engage stakeholders; ensure transparency & records management"
    ],
    "Identify": [
...
def suggest_nist(incident_type: str, description: str):
    ...
    for k, v in NIST_KB.items():
        if k in it or k in description.lower():
            seed.extend(v)
    if not seed:
        seed = CSF_FUNCTIONS[:]  # default to all CSF functions
...
# (Fix for your error) — Properly formed dictionary with straight quotes & commas
scenario_summaries = {
    ...
    "San Diego smart streetlights and surveillance": [
        ...
        "They were used for police surveillance without public consent, raising concerns about transparency, trust, and misuse."
    ],
...
- **Backbone:** NIST CSF 2.0 (Govern/Identify/Protect/Detect/Respond/Recover) + Principlist ethics (Beneficence, Non-maleficence, Autonomy, Justice, Explicability).
...
with colB:
    ...
    selected_nist = st.multiselect("", CSF_FUNCTIONS, default=suggested_nist)
